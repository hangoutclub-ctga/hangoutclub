...
import { SlidersHorizontal, Trash2, ToggleRight, Check, Search, CircleX, X } from "lucide-react"
...
export function DataTableToolbar<TData>({
  ...
  const getSelectedRowIds = () => {
    return table.getFilteredSelectedRowModel().rows.map(row => (row.original as Class).id);
  }
           ...
           <DropdownMenu>
            ...
            <DropdownMenuContent align="start">
                 <DropdownMenuSub>
                    ...
                    <DropdownMenuSubContent>
                        <DropdownMenuItem onSelect={() => onBulkUpdate(getSelectedRowIds(), { status: 'Ativa' } as any)}>
                            <Check className="mr-2 h-4 w-4 text-green-500" /> Ativa
                        </DropdownMenuItem>
                        <DropdownMenuItem onSelect={() => onBulkUpdate(getSelectedRowIds(), { status: 'Inativa' } as any)}>
                            <Check className="mr-2 h-4 w-4 text-red-500" /> Inativa
                        </DropdownMenuItem>
                    </DropdownMenuSubContent>
                </DropdownMenuSub>
                <DropdownMenuSeparator />
